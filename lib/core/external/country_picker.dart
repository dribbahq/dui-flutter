...
class CountryPicker extends StatefulWidget {
  final ValueChanged<CountryPicked?>? onChanged;
  ...
  State<StatefulWidget> createState() {
    ...
    List<CountryPicked> elements = jsonList
        .map((s) => CountryPicked(
              countryName: s['name'],
              countryExtension: s['code'],
              countryCode: s['dial_code'],
              //flagImage: 'assets/flags/${s['code']}.svg',
            ))
        .toList();
...
class _CountryPicker extends State<CountryPicker> {
  CountryPicked? selectedItem;
  List<CountryPicked> elements = [];
  List<CountryPicked> favoriteElements = [];

  _CountryPicker(this.elements);
    ...
    if (widget.initialSelection != null) {
      selectedItem = elements.firstWhere(
          (e) =>
              ...
              (e.countryExtension == widget.initialSelection.toString()),
          orElse: () => elements[0]);
    } else {
    ...
    favoriteElements = elements
        .where((e) =>
            widget.favorite!.firstWhereOrNull((f) =>
                e.countryCode == f.toUpperCase() ||
                e.countryExtension == f.toString()) !=
            null)
        .toList();
        ...
        content: SizedBox(
          ...
          child: CupertinoPicker(
            ...
            onSelectedItemChanged: (int index) {
              if (index < favoriteElements.length) {
                selectedItem = favoriteElements[index];
              } else if (index != favoriteElements.length) {
                selectedItem = elements[index - favoriteElements.length - 1];
              }
  ...
  void _publishSelection(CountryPicked? e) {
  ...
  List<Widget> _generateItems() {
    ...
    for (CountryPicked element in favoriteElements) {
      items.add(Row(
        mainAxisAlignment: MainAxisAlignment.start,
    ...
    for (CountryPicked element in elements) {
      items.add(Row(
        mainAxisAlignment: MainAxisAlignment.start,
        crossAxisAlignment: CrossAxisAlignment.center,
        children: <Widget>[
